                      ...
                      vec3 const& p_camera)
{
    ...
    float k_specular = pow(clamp(dot(s,t))/(norm(s)*norm(t)),m);
    float k_diffus =clamp(dot(normal,d))/(norm(normal)*norm(d)) ;
    color c = input* (k_ambiant + k_diffus)+ light_color * k_specular;
    return c;
